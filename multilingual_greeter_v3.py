...
greetings_dict = {
    1 : ["Hello", "Hi", "Whaddup my buddy?"],
    2 : ["Hola", "Buenos Dias", "¿Cómo Estás?"],
    3 : ["Ola", "Oi", "Bom dia"],
}
...
def language_adder():
    ...
    greetings: list[str] = []
    while True:
        greeting_request = input("Please type a greeting in " + new_language + ". When done type \"Exit\":")
        if greeting_request.lower() == "exit":
            break
        greetings.append(greeting_request)
    greetings_dict[new_key] = greetings
...
def greet(name: str, greetings_options: Dict[int, str], lang_choice: int) -> None:
    greetings = greetings_options[lang_choice]
